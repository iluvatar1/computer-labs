...
\begin{itemize}
  ...
  \item La distribución a instalar es Slackware, Versión 13.0 a la
    fecha.
  ...
  \item Usando dyndns y el cleinte ddclient, se asignará el nombre
...
Al final de esta secci\'on se supone que el ususario ha instalado exitosamente Slackware en cada
...
/mirror/slackware-13.0 . The files for tftp will be on /tftpboot/slackware-13.0.

\begin{itemize}
\item Export the mirrror on NFS: Add \verb+/mirror/Slackware 192.168.123.0/24(ro,sync,insecure,all_squash)'+ 
to \verb+/etc/exports+
\item Create the mirror directory: \verb+mkdir -p   /mirror/slackware-13.0+
\item \verb$chmod +x /etc/rc.d/rc.nfsd $
\item \verb+/etc/rc.d/rc.nfsd restart+
\item \verb+mkdir /tftpboot+
\item \verb+mkdir /tftpboot/slackware-13.0+
\item \verb+mkdir /tftpboot/slackware-13.0/pxelinux.cfg+
\item \verb+cp /usr/lib/syslinux/pxelinux.0 /tftpboot/slackware-13.0+
\item \verb+cp /mirror/slackware-13.0/isolinux/message.txt /tftpboot/slackware-13.0/+
\item \verb+cp /mirror/slackware-13.0/isolinux/f2.txt /tftpboot/slackware-13.0/+
\item \verb+cp /mirror/slackware-13.0/isolinux/f3.txt /tftpboot/slackware-13.0/+
\item \verb+cp -a /mirror/slackware-13.0/kernels  /tftpboot/slackware-13.0/+
\item \verb+cp /mirror/slackware-13.0/usb-and-pxe-installers/pxelinux.cfg_default \+\\
\verb+ /tftpboot/slackware-13.0/pxelinux.cfg/default+
\item \verb+cp /mirror/slackware-13.0/isolinux/initrd.img /tftpboot/slackware-13.0/+
\end{itemize} 
...
dhcp-option-force=210,/tftpboot/slackware-13.0/
dhcp-boot=pxelinux.0
enable-tftp
tftp-root=/tftpboot/slackware-13.0
\end{verbatim}

\subsection{Clientes}
Each client should try to boot up from the net, and if the client is
able to find the pxe server, all the installation will proceed without problem.
...
Editar el archivo \verb+/etc/dhcpc/dhcpcd.sh + . En este archivo
...
\begin{verbatim}
/dev/sda5        /home            reiserfs    defaults,usrquota         1   2
\end{verbatim}
...
successfull, \verb+scanimage -L+ . They say it is a
